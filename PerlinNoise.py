...
if __name__ == '__main__':
    ...
    # instance (1) 
    pn = PerlinNoise(num_octaves=5, speed_up_rate=0.0, tc=2.0)
    su= pn.make(length = int( 250 * 0.001 / (1.0 / pn.sampling_rate)) )  # 250ms
    pn.plot_waveform('Waveform (constant velocity)')
    wavwrite( 'i_noise.wav', pn.sampling_rate, ( su * 2 ** 15).astype(np.int16))
    print ('save i_noise.wav')
    
    # apply HPF(low freq cut) to narrow spectrum band wide
    hpf1= Class_IIR1(fc=2400.0, btype='high', n_order=3)
    ydh= hpf1.filtering( su)
    sub_plot_waveform(ydh,'Waveform (narrow band)', pn.sampling_rate)
    wavwrite( 'i_noise_narrow.wav', pn.sampling_rate , ( ydh * 2 ** 15).astype(np.int16))
    print ('save i_noise_narrow.wav')
